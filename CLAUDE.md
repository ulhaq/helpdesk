...
- **AI assistant** - Claude (`backend/src/helpdesk/assistant.py`) drafts agent replies (`POST /v1/tickets/{id}/reply-suggestion`) and answers widget questions (`POST /v1/widget/<slug>/answers`) from published articles with citations; off without `ANTHROPIC_API_KEY`, metered by `ai_requests_per_month`. Reply drafts also use **internal knowledge** - team-only documents pasted or uploaded (.txt/.md/.pdf/.docx) at `/knowledge-base/internal`; widget answers never do. Context comes from `backend/src/helpdesk/retrieval.py`: requests get the best-matching sections (`knowledge_chunk`), ranked by Postgres full-text search fused with pgvector semantic search when an embedding provider is configured (`HELPDESK_EMBEDDING_*`: any OpenAI-compatible server such as a local Qwen3-Embedding, or Voyage). Only without an embedding provider are small knowledge bases sent whole (prompt-cached, `HELPDESK_AI_FULL_CONTEXT_MAX_CHARS`), since keyword search alone misses paraphrases. Postgres runs from the `pgvector/pgvector:pg16` image. Each request is logged to `ai_request_log` to spot retrieval misses.
- **Worker loops** - auto-closes tickets left resolved (`HELPDESK_AUTO_CLOSE_RESOLVED_AFTER_DAYS`), deletes old AI request logs (`HELPDESK_AI_REQUEST_LOG_RETENTION_DAYS`) and embeds new or changed knowledge chunks.
