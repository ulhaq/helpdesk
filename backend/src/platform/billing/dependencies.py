...
async def require_within_limit(
    repos: RepositoryManager, organization_id: int, metric: StrEnum
) -> None:
    """
    Raise LIMIT_EXCEEDED when the organisation has consumed its plan allocation
    for `metric` this period. For code paths without an authenticated user
    (public endpoints, workers); routes should prefer `require_limit`.
    """
    limit = await repos.plan_setting.get_for_organization(organization_id, metric)
...
def require_limit(metric: StrEnum) -> Callable:
    ...
    async def _check(
        ...
        await require_within_limit(repos, current_user.organization_id, metric)
