...
| `src/helpdesk/models/` | `Contact`, `Ticket`, `TicketMessage` (org-scoped, soft-deletable), `KbCategory`/`KbArticle`, `KnowledgeDocument` (internal knowledge, team only), `KnowledgeChunk` (article or document sections for AI retrieval, with embeddings), `AiRequestLog`, `SupportSite` (one per org: public slug, branding, widget/help center switches) |
...
| `src/helpdesk/retrieval.py`, `kb_chunks.py` | RAG context: `KnowledgeRetriever` sends the best-matching sections within a budget - or, only when no embedding model is configured, a small knowledge base whole (below `ai_full_context_max_chars`, prompt-cached); `include_internal=True` (reply drafts only) adds internal documents. Sections are ranked by full-text search and, with an embedder, pgvector similarity, merged by reciprocal rank fusion; embedding failures fall back to full-text search. Text is split on every save: Markdown at headings, plain text in overlapping windows |
| `src/helpdesk/repositories/knowledge.py` | `KnowledgeChunkRepository`: `search` (GIN expression index that must match `_SEARCH_VECTOR`), `nearest` (HNSW index, `hnsw.iterative_scan`) and embedding bookkeeping. Both searches have SQLite fallbacks for the test suite, so the Postgres queries are only exercised against a real database |
| `src/helpdesk/embeddings.py` | `Embedder` protocol with `OpenAICompatibleEmbedder` (Ollama/vLLM/TEI/llama.cpp, e.g. a local Qwen3-Embedding) and `VoyageEmbedder`; `get_embedder` dependency (None when unconfigured; tests override it). Vectors are fixed at `EMBEDDING_DIMENSIONS` (1024) |
| `src/helpdesk/services/knowledge.py`, `extraction.py` | Internal knowledge documents (`/v1/knowledge/documents`, `manage:kb`): pasted text or uploads (.txt/.md/.pdf/.docx; text extracted off the event loop, original files aren't stored). `POST /v1/knowledge/search` runs `KnowledgeRetriever.explain` - the retrieval ranking (keyword/semantic rank, distance, what would reach Claude, pending embeddings) without calling Claude |
| `src/helpdesk/hooks.py` | `MEMBER_REMOVED` → unassign the member's tickets |
| `src/helpdesk/worker.py` | `run_auto_close_loop` - closes tickets left resolved, recording `worker_run` rows; `run_ai_request_log_retention_loop` - daily purge of `ai_request_log` rows (they hold customer text); `run_embedding_loop` - embeds chunks without a vector from the current model, so a model change re-embeds everything |
| `src/helpdesk/config.py` | `HelpdeskSettings` (`env_prefix="helpdesk_"`: contact token lifetime, auto-close, AI model/effort, retrieval budget, embeddings, uploads, AI log retention) |
