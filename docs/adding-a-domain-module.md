...
How to add a new product/domain package (here called `acme`) next to - or instead of - the current `helpdesk` product, on both sides of the stack. The rule everywhere: **the platform never imports your domain; your domain imports the platform; only the assembly layer knows both.**

## Backend (`backend/src/acme/`)

### 1. Create the package

Mirror the layout of `src/helpdesk/`:
...
Conventions that carry over from `helpdesk`:
...
- Unauthenticated (customer-facing) routers resolve the tenant from something public - `helpdesk` uses a support site slug - scope every repository to it, and rate limit each route (see `src/helpdesk/routers/widget.py`).
...
```python
ALL_PERMISSIONS = [*core_enums.Permission, *HelpdeskPermission, *AcmePermission]
PERMISSION_DESCRIPTIONS = {**core_enums.PERMISSION_DESCRIPTIONS,
                           **HELPDESK_PERMISSION_DESCRIPTIONS,
                           **ACME_PERMISSION_DESCRIPTIONS}
# merge ACME_DEFAULT_ROLE_PERMISSIONS into DEFAULT_ROLES the same way helpdesk does
...
New permissions shift the counts asserted in `tests/api/test_permission.py`. Add tests under `backend/tests/` (API tests get the seeded multi-tenant fixtures from `conftest.py` for free; your permissions/roles are seeded because `conftest` imports the composed sets from `src.bootstrap`).
...
Nav items can set `permission` so users who can't open the page don't see it. If your module owns the authenticated home page, set it from `main.ts` via `configureApp({ homeRoute: '/acme' })`. Pages for people without an account (embeds, public portals) use `meta: { layout: bare, public: true }` and a separate API client that never sends agent credentials.
...
Same steps - but first delete `src/helpdesk/` on both sides and strip its assembly references: `bootstrap.py`, `main.py` router includes, `alembic/env.py`, `worker.py` (backend); `main.ts`, `vite.config.ts`, `plugins/i18n.ts`, the ESLint pattern (frontend). The platform packages need no changes at all. Also drop the helpdesk migration (`alembic/versions/*_helpdesk_core.py`) and the `tickets_per_month` / `kb_articles` plan limits it seeds, the helpdesk tests, `frontend/public/widget.js` and the `/widget/` block in `frontend/nginx.conf.template`, and update `src/brand.ts`, `planComparisonRows` and `planDescriptions` for the new product.
