...
export default [
  ...
  {
    // Standalone browser scripts served as-is from public/ (e.g. the support
    // widget loader embedded on customers' sites): no bundler, no modules.
    files: ['public/**/*.js'],
    languageOptions: {
      sourceType: 'script',
    ...
    rules: {
      'no-restricted-imports': [
        'error',
        {
          patterns: [
            {
              group: ['@/helpdesk', '@/helpdesk/**', '**/helpdesk/**'],
              message: 'Platform code must not import from the product domain (src/helpdesk).',
            },
          ],
        },
      ],
