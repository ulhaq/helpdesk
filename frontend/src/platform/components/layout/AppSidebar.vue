...
import { navItemsFor, type NavGroup } from '@/platform/navigation'
...
function visibleNavItems(group: NavGroup) {
  return navItemsFor(group)
    .filter((i) => !i.permission || profileStore.hasPermission(i.permission))
...
const navGroups = computed(() => [
  {
    label: t('nav.main'),
    items: visibleNavItems('main'),
  },
  {
    label: t('nav.manage'),
    items: visibleNavItems('manage'),
  },
])
