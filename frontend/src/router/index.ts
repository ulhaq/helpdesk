...
declare module 'vue-router' {
  interface RouteMeta {
    layout?: 'auth' | 'dashboard' | 'auto' | 'landing' | 'bare'
    /**
     * Customer-facing page (e.g. an embedded support widget): renders on any
     * host and never restores or requires an agent session.
     */
    public?: boolean
...
export function installRouterGuards(router: Router): void {
  router.beforeEach(async (to: RouteLocationNormalized) => {
    ...
    // Public pages are opened on whatever host embeds or links them, and must
    // not touch the agent session (no refresh-cookie call, no login bounce).
